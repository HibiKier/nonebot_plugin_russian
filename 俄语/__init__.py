...
__usage__ = """俄罗斯轮盘帮助：
    ...
    【注：同一时间群内只能有一场对决】【金币可以用签到获得】
"""

scheduler = require("nonebot_plugin_apscheduler").scheduler


sign = on_command("签到", permission=GROUP, priority=5, block=True)
...
async def _(bot: Bot, event: GroupMessageEvent, arg: Message = CommandArg()):
    ...
    if is_number(count):
        count = int(count)
        if not (0 < count < 7 - russian_manager.get_current_bullet_index(event)):
            await shot.reject(
                f"你不能开{count}枪，大于或小于剩余的子弹数量，"
